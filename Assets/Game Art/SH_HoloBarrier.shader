    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
                ...
                float4 node_8782 = _Time + _TimeEditor;
                float2 node_6235 = (i.uv0+node_8782.g*float2(0,-0.2));
                float4 _Noise_var = tex2D(_Noise,TRANSFORM_TEX(node_6235, _Noise));
                float node_2783 = 6.0;
                float2 node_2448 = ((i.uv0+node_8782.g*float2(-0.02,0.03))*node_2783);
                float4 _TilingTexture_copy_var = tex2D(_TilingTexture_copy,TRANSFORM_TEX(node_2448, _TilingTexture_copy));
                float2 node_8877 = ((i.uv0+node_8782.g*float2(0.01,-0.04))*node_2783);
                float4 _TilingTexture3_var = tex2D(_TilingTexture3,TRANSFORM_TEX(node_8877, _TilingTexture3));
                float node_9140 = (_TilingTexture_copy_var.r+_TilingTexture3_var.b);
                float node_6506 = (1.0 - _FlimmerPower);
                float node_626 = clamp((((pow((1.0 - (_Noise_var.g*(_Noise_var.r*_Noise_var.b))),8.0)-(1.0 - (_IntersectionPower*2.0)))+_IntersectionPower)*pow(node_9140,node_6506)),0.2,0.8);
                clip(node_626 - 0.5);
////// Lighting:
////// Emissive:
                float node_3392 = 24.0;
                float2 node_1653 = ((i.uv0+node_8782.g*float2(0,0.01))*node_3392);
                float4 _TilingTexture_var = tex2D(_TilingTexture,TRANSFORM_TEX(node_1653, _TilingTexture));
                float2 node_3767 = ((i.uv0+node_8782.g*float2(0,0.005))*node_3392);
                float4 _TilingTexture2_var = tex2D(_TilingTexture2,TRANSFORM_TEX(node_3767, _TilingTexture2));
                float3 node_4237 = saturate(( _MainColor.rgb > 0.5 ? (1.0-(1.0-2.0*(_MainColor.rgb-0.5))*(1.0-pow((lerp((_TilingTexture_var.r+_TilingTexture2_var.g),dot((_TilingTexture_var.r+_TilingTexture2_var.g),float3(0.3,0.59,0.11)),1.0)*float3(0.4411765,0.4411765,0.4411765)),node_6506))) : (2.0*_MainColor.rgb*pow((lerp((_TilingTexture_var.r+_TilingTexture2_var.g),dot((_TilingTexture_var.r+_TilingTexture2_var.g),float3(0.3,0.59,0.11)),1.0)*float3(0.4411765,0.4411765,0.4411765)),node_6506)) ));
                float3 emissive = (((node_4237*pow(node_9140,node_6506))+saturate(( _IntersectionColor.rgb > 0.5 ? (1.0-(1.0-2.0*(_IntersectionColor.rgb-0.5))*(1.0-(1.0 - saturate((sceneZ-partZ)/_IntesectionDistance)))) : (2.0*_IntersectionColor.rgb*(1.0 - saturate((sceneZ-partZ)/_IntesectionDistance))) )))*node_626);
